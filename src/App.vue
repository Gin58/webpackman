<template>
  <div>
    <nav class="relative flex flex-wrap items-center justify-between px-2 py-3 bg-pink-500 mb-3">
      <div class="container px-4 mx-auto flex flex-wrap items-center justify-between">
        <div class="w-full relative flex justify-between lg:w-auto  px-4 lg:static lg:block lg:justify-start">
          <a class="text-sm font-bold leading-relaxed inline-block mr-4 py-2 whitespace-nowrap uppercase text-white" href="#pablo">
            pink Color
          </a>
          <button class="text-white cursor-pointer text-xl leading-none px-3 py-1 border border-solid border-transparent rounded bg-transparent block lg:hidden outline-none focus:outline-none" type="button" v-on:click="toggleNavbar()">
            <font-awesome-icon icon="bars" />
          </button>
        </div>
        <div v-bind:class="{'hidden': !state.showMenu, 'flex': state.showMenu}" class="lg:flex lg:flex-grow items-center">
          <ul class="flex flex-col lg:flex-row list-none ml-auto">
            <li class="nav-item">
              <router-link 
                to="/about" 
                exact-active-class="active"
                class="px-3 py-2 flex items-center text-xs uppercase font-bold leading-snug text-white hover:opacity-75"
              >
                <font-awesome-icon :icon="['fab', 'facebook-square']"  class="text-lg leading-lg text-white opacity-75" />
                <span class="ml-2">Share</span>
              </router-link>
            </li>
            <li class="nav-item">
              <a class="px-3 py-2 flex items-center text-xs uppercase font-bold leading-snug text-white hover:opacity-75" href="#pablo">
                <font-awesome-icon :icon="['fab', 'twitter']"  class="text-lg leading-lg text-white opacity-75" />
                <span class="ml-2">Tweet</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="px-3 py-2 flex items-center text-xs uppercase font-bold leading-snug text-white hover:opacity-75" href="#pablo">
                <font-awesome-icon :icon="['fab', 'pinterest']"  class="text-lg leading-lg text-white opacity-75" />
                <span class="ml-2">Pin</span>
              </a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
    <router-view/>
  </div>
</template>

<script lang="ts">
import { defineComponent, reactive } from '@vue/composition-api'
import sample from './components/sample.vue'

export default defineComponent({
  components: {
    sample
  },
  setup() {
    const state = reactive<{ showMenu: boolean }>({
      showMenu: false
    })

    const toggleNavbar = () => {
      state.showMenu = !state.showMenu
    }

    return {
      state,
      toggleNavbar
    }
  }
})
</script>
<style lang='scss'>
.active {
  color: red;
}
</style>
